...
class Preprocess_using_grobid:
    ...
    def write_images(self, pdf_path, pages):
        ...
        # added thread count for faster speed inference
        images = convert_from_path(
            pdf_path, first_page=1, use_pdftocairo=True, fmt="png", thread_count=128
        )
        ...
        # #     # image.show()
        # #     image_name = path + "/" + "image_" + str(i + 1) + ".png"
        # #     write_image(image, image_name)

        # multithreaded
        # Parallel(n_jobs=-2, backend="threading", prefer="threads", verbose=20)(
        #     delayed(write_image)(image, path + "/" + "image_" + str(i + 1) + ".png")
        #     for i, image in enumerate(images)
        # )
    ...
    def rescale(self, grobid_xml, coordinates, page_no, resolution, images):
        ...
        for image, i in zip(images, range(page_no)):
            size = image.size

            scale_x = size[1] / resolution[i][1]
            scale_y = size[0] / resolution[i][0]
    ...
    def visualize_boxes(self, dict_coords, images):
        ...
        for k, v in tqdm(dict_coords.items()):
            for element in v:
                image, text = self.draw_rectangle(images, box=element)
                track.append(image)
                texts.append(text)
                coords.append(element)
    ...
    def draw_rectangle(self, images, box):
        x1, y1, x2, y2 = (
            math.floor(box[1][0]),
            math.floor(box[1][1]),
            math.ceil(box[2][0]),
    ...
    def fit(self, grobid_xml, show_results=False):
        ...
        images = self.write_images(
            grobid_xml.replace(".tei.xml", ".pdf"), header_info[-1]
        )
        ...
        # make dictionary divided in pages
        ...
        if show_results == True:
            ...
            return images, self.scales, dict_coords
